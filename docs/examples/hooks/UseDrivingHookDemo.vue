...
const { container, hasKey, map } = useHookDemoMap({}, { plugins: ['AMap.Driving'] })

const driving = useDriving(computed(() => ({
  map: map.value,
...
async function runSearch() {
  ...
  loading.value = true
  error.value = null
  result.value = await driving.search(origin.value, destination.value)
  loading.value = false
  if (!result.value)
    error.value = '未找到可用的驾车路线。'
}
...
<template>
  <div class="amap-demo amap-demo--split">
    <div v-if="!hasKey" class="amap-demo__placeholder">
      Set <code>VITE_AMAP_KEY</code> to preview <code>useDriving</code>.
    </div>
    <template v-else>
