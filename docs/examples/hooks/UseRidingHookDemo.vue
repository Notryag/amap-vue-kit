...
const destination = ref('北海公园')
const loading = ref(false)
const error = ref<string | null>(null)
const result = shallowRef<AMap.Riding.SearchResult | null>(null)

const panelRef = ref<HTMLDivElement | null>(null)

const { container, hasKey, map } = useHookDemoMap({}, { plugins: ['AMap.Riding'] })

const riding = useRiding(computed(() => ({
  map: map.value,
...
async function runSearch() {
  ...
  loading.value = true
  error.value = null
  result.value = await riding.search(origin.value, destination.value)
  loading.value = false
  if (!result.value)
    error.value = '未找到可用的骑行路线。'
}
...
<template>
  <div class="amap-demo amap-demo--split">
    <div v-if="!hasKey" class="amap-demo__placeholder">
      Set <code>VITE_AMAP_KEY</code> to preview <code>useRiding</code>.
    </div>
    <template v-else>
