...
import type { DrivingEndpoint, UseDrivingOptions } from '@amap-vue/hooks'
...
const props = defineProps({
  origin: {
    type: [String, Array, Object] as PropType<DrivingEndpoint | undefined>,
    default: undefined,
  },
  destination: {
    type: [String, Array, Object] as PropType<DrivingEndpoint | undefined>,
...
const drivingOptions = computed<UseDrivingOptions>(() => {
  const base: UseDrivingOptions = {
    ...(props.options ?? {}),
    map: props.map,
...
const drivingApi = useDriving(drivingOptions)
const driving = drivingApi.driving
...
async function runSearch(origin: DrivingEndpoint | undefined = resolveOrigin(), destination: DrivingEndpoint | undefined = resolveDestination()) {
  if (!origin || !destination) {
  ...
  const response = await drivingApi.search(origin, destination)
  loading.value = false
  result.value = response
  if (!response)
    handleError('驾车路线规划失败，请稍后重试。')
