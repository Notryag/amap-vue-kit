...
import type { UseToolBarOptions } from '@amap-vue/hooks'
...
const props = defineProps({
  ...
  options: {
    type: Object as PropType<Partial<UseToolBarOptions>>,
    default: () => ({}),
  },
})
...
if (!context)
  warn('<AmapToolBar> must be used inside <AmapMap>.')

const controlOptions = computed<UseToolBarOptions>(() => {
  const base: UseToolBarOptions = {
    ...props.options,
...
const controlApi = context ? useToolBar(() => context.map.value, controlOptions) : null
