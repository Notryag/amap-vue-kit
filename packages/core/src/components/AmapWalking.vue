...
import type { UseWalkingOptions, WalkingEndpoint } from '@amap-vue/hooks'
...
const props = defineProps({
  origin: {
    type: [String, Array, Object] as PropType<WalkingEndpoint | undefined>,
    default: undefined,
  },
  destination: {
    type: [String, Array, Object] as PropType<WalkingEndpoint | undefined>,
  ...
  options: {
    type: Object as PropType<Partial<AMap.WalkingOptions>>,
    default: () => ({}),
  },
  map: {
...
const emit = defineEmits<{
  ready: [walking: AMap.Walking]
  search: [payload: { origin: WalkingEndpoint, destination: WalkingEndpoint, result: AMap.WalkingResult | null }]
  error: [message: string]
}>()

const panelRef = ref<HTMLElement | null>(null)

const walkingOptions = computed<UseWalkingOptions>(() => {
  const base: UseWalkingOptions = {
    ...(props.options ?? {}),
    map: props.map,
...
const walkingApi = useWalking(walkingOptions)
const walking = walkingApi.walking
...
async function runSearch(origin: WalkingEndpoint | undefined = resolveOrigin(), destination: WalkingEndpoint | undefined = resolveDestination()) {
  if (!origin || !destination) {
  ...
  const response = await walkingApi.search(origin, destination)
  loading.value = false
  result.value = response
  if (!response)
    handleError('步行路线规划失败，请稍后重试。')
